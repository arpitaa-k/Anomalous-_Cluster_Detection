...
SCORES_PATH = Path("results/graph_deviation_temporal_scores.csv")
OUTPUT_PATH = Path("results/graph_deviation_score_timeseries.png")
...
def get_node_scores(df: pd.DataFrame, node: str):
    ...
    return node_df["window_start"], node_df["graph_deviation_score_raw"]
...
def choose_plot_nodes(df: pd.DataFrame) -> list[str]:
    ...
    top_nodes = (
        df.groupby("node", as_index=False)["graph_deviation_score_raw"]
        .mean()
        .sort_values("graph_deviation_score_raw", ascending=False)["node"]
        .astype(str)
        .tolist()
    )
...
def main() -> None:
    if not SCORES_PATH.exists():
        raise FileNotFoundError(
            f"Temporal graph deviation file not found at {SCORES_PATH}. Run: python graph_deviation_temporal.py"
        )

    df = pd.read_csv(SCORES_PATH)
    if "window_start" not in df.columns:
        raise ValueError("window_start column not found in graph deviation temporal CSV")
    ...
    plt.ylabel("Graph Deviation Raw Score")
    plt.title("Graph Deviation Score Over Time for Key Nodes")
    plt.legend()
